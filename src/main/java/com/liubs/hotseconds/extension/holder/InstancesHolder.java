...
public class InstancesHolder {
    ...
    private static final Set<Class<?>> alreadyHold = Collections.synchronizedSet(new HashSet<>());
    ...
    public static void insertObjectCacheInConstructor(CtClass ctClass) {
        try{
            for (CtConstructor constructor : ctClass.getDeclaredConstructors()) {
                ...
                constructor.insertAfter("com.liubs.hotseconds.extension.holder.InstancesHolder.insertHolder(this);");
                src.append("}");
                constructor.insertAfter(src.toString());
            }
        }catch (Exception e) {
            logger.error("insertObjectCacheInConstructor err,ctClass={}",e,ctClass);
        }

    }

    public static void insertHolder(Object obj){
        if(alreadyHold.contains(obj.getClass())) {
            return;
        }
